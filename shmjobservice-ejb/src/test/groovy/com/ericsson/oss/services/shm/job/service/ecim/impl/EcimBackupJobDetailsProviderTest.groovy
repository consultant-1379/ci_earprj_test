...
class EcimBackupJobDetailsProviderTest extends CdiSpecification{
    ...
    def "Check for Securue backup for the backup Jobs having the password and userlabel in the JobConfiguration"(encrypSupported,jsonResponseContainsSecureBakcup){
        given :"JobConfiguration"
        JobConfiguration jobConfiguration = new JobConfiguration();
        when:"When JobSummary Rest end Request for response"
        JobConfigurationDetails jobConfigurationDetails = backupJobDetailsProvider.getJobConfigParamDetails(buildJobConfigToPrepareJobSummary(encrypSupported,jobConfiguration), NETYPE)
        ...
        for(ActivityInfo activityInfo:activtyInfoList){
            if(activityInfo.getActivityName().equals(ShmConstants.CREATE_BACKUP)){
                ...
                for (JobProperty jobProperty : createBackupJobPropertyList){
                    ...
                    if (!jobProperty.getKey().equals(JobPropertyConstants.USER_LABEL) && jobProperty.getKey().equals(ShmConstants.DISPLAY_USER_LABEL)){
