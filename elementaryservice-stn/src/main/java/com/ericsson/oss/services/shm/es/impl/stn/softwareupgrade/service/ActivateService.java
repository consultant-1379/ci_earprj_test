...
import com.ericsson.oss.services.shm.es.impl.stn.softwareupgrade.common.StnSoftwareUpgradeActivateEventSender;
 ...
 * 
 */
@EServiceQualifier("STN.UPGRADE.upgrade")
...
public class ActivateService implements Activity, ActivityCallback {
    ...
    @Inject
    private StnSoftwareUpgradeActivateEventSender stnSoftwareUpgradeEventSender;
    ...
    /**
     * Precheck for Activate activity of Software Upgrade on STN nodes.
     * 
     * @param activityJobId
    ...
    public ActivityStepResult precheck(final long activityJobId) {
        ...
        LOGGER.debug("ActivityJob ID - [{}] : precheck of Activate/Upgrade activity of software upgrade on STN node.", activityJobId);
        ...
        try {
            ...
            //TBAC Validation
            final boolean isUserAuthorized = activityJobTBACValidator.validateTBAC(activityJobId, neJobStaticData, jobStaticData, ActivityConstants.UPGRADE);
            if (!isUserAuthorized) {
                activityStepResult.setActivityResultEnum(ActivityStepResultEnum.PRECHECK_FAILED_SKIP_EXECUTION);
                return activityStepResult;
            }

            activityUtils.prepareJobLogAtrributesList(jobLogs, String.format(JobLogConstants.PRECHECK_SUCCESS, ActivityConstants.UPGRADE), new Date(), JobLogType.SYSTEM.toString(),
            ...
            activityStepResult.setActivityResultEnum(ActivityStepResultEnum.PRECHECK_FAILED_SKIP_EXECUTION);
            jobLogUtil.prepareJobLogAtrributesList(jobLogs, String.format(JobLogConstants.PRE_CHECK_FAILURE, ActivityConstants.UPGRADE, exception.getMessage()), new Date(),
    ...
    public void execute(final long activityJobId) {
        ...
        try {
            ...
            if (stnUpgradeInformation == null) {
                ...
                stnJobActivityService.failActivity(activityJobId, jobLogs, activityUtils.getJobEnvironment(activityJobId), ActivityConstants.UPGRADE, processVariables);
                return;
            }
            LOGGER.debug("ActivityJob ID - [{}] : executing Activate/Upgrade activity of software upgrade on node:{}", activityJobId, stnUpgradeInformation.getNodeFdn());
            jobContext = stnUpgradeInformation.getJobEnvironment();
            systemRecorder.recordCommand(SHMEvents.STN_UPGRADE_SERVICE, CommandPhase.STARTED, stnUpgradeInformation.getNeName(), stnUpgradeInformation.getNodeFdn(), StnJobConstants.UPGRADE_ACTIVITY);
            ...
            activityUtils.prepareJobLogAtrributesList(jobLogs, String.format(JobLogConstants.ACTIVITY_INITIATED, ActivityConstants.UPGRADE), new Date(), JobLogType.SYSTEM.toString(),
                    JobLogLevel.INFO.toString());
            LOGGER.debug("ActivityJob ID - [{}] : Execute of Activate/Upgrade activity is triggered successfully on Node {}.", activityJobId, stnUpgradeInformation.getNodeFdn());
            ...
            logMessage = String.format(JobLogConstants.ACTIVITY_FAILED, ActivityConstants.UPGRADE);
            activityUtils.prepareJobLogAtrributesList(jobLogs, logMessage, new Date(), JobLogType.SYSTEM.toString(), JobLogLevel.ERROR.toString());
            stnJobActivityService.failActivity(activityJobId, jobLogs, jobContext, ActivityConstants.UPGRADE, processVariables);
        } catch (final Exception exception) {
            LOGGER.error("Failed to execute Activate/Upgrade activity and cannot be proceeded with activity with activityId {} for Node :{} Reason : {}", activityJobId,
                    stnUpgradeInformation.getNodeFdn(), exception);
            logMessage = String.format(JobLogConstants.ACTIVITY_FAILED, ActivityConstants.UPGRADE);
            activityUtils.prepareJobLogAtrributesList(jobLogs, logMessage, new Date(), JobLogType.SYSTEM.toString(), JobLogLevel.ERROR.toString());
            stnJobActivityService.failActivity(activityJobId, jobLogs, jobContext, ActivityConstants.UPGRADE, processVariables);
        }
    ...
    private void sendSoftwareUpgradeActivityRequest(final long activityJobId, final StnJobInformation stnUpgradeInformation, final JobActivityInfo jobActivityInformation) {
        final List<Map<String, Object>> jobLogs = new ArrayList<>();
        LOGGER.debug("ActivityJob ID - [{}] : Activate/Upgrade activity of STN software upgrade job is triggered. Software Upgrade Job Information : {} ", activityJobId, stnUpgradeInformation);
        activityUtils.prepareJobLogAtrributesList(jobLogs, String.format(StnJobConstants.ACTION_ABOUT_TO_TRIGGER, ActivityConstants.UPGRADE, stnUpgradeInformation.getNodeFdn()), new Date(),
                JobLogType.SYSTEM.toString(), JobLogLevel.INFO.toString());
        final String nodeFdn = stnUpgradeInformation.getNodeFdn();
        final String subscriptionKey = StnJobConstants.SUBSCRIPTION_KEY_DELIMETER + stnUpgradeInformation.getNodeFdn() + StnJobConstants.SUBSCRIPTION_KEY_DELIMETER + StnJobConstants.UPGRADE_ACTIVITY
        ...
        if (nodeFdn != null) {
            ...
            LOGGER.debug("ActivityJob ID - [{}] : Subscription key for Activate/Upgrade action is : {}", activityJobId, subscriptionKey);
            activityUtils.subscribeToMoNotifications(subscriptionKey, activityJobId, jobActivityInformation);
            jobUpdateService.readAndUpdateRunningJobAttributes(activityJobId, null, jobLogs);
            stnSoftwareUpgradeEventSender.sendSoftwareUpgradeActionRequest(activityJobId, ActivityConstants.UPGRADE, nodeFdn, eventAttributes);
        } else {
            jobUpdateService.readAndUpdateRunningJobAttributes(activityJobId, null, jobLogs);
    ...
    private void handleActivityProgressNotification(final SHMCommonCallBackNotificationJobProgressBean notification, final StnJobInformation stnUpgradeInformation,
        ...
        LOGGER.debug("ActivityJob ID - [{}] : Job notification received for Activate activity is: {}", activityJobId, notification.getCommonNotification());
        eventAttributes.put(ShmConstants.NE_NAME, stnUpgradeInformation.getNeName());
        eventAttributes.put(ActivityConstants.ACTIVITY_JOB_ID, activityJobId);
        ...
        try {
            ...
            if (ShmConstants.SUCCESS.equalsIgnoreCase(result)) {
                activityUtils.prepareJobLogAtrributesList(jobLogs, "Activate Activity is Success",
                ...
                systemRecorder.recordCommand(SHMEvents.STN_UPGRADE_SERVICE, CommandPhase.FINISHED_WITH_SUCCESS, stnUpgradeInformation.getNeName(), stnUpgradeInformation.getNodeFdn(),
                        StnJobConstants.UPGRADE_ACTIVITY);
                ...
                activityAndNEJobProgressPercentageCalculator.updateActivityJobProgressPercentage(activityJobId, jobProperties, jobLogs, progressPercent);
                activityAndNEJobProgressPercentageCalculator.updateNEJobProgressPercentage(stnUpgradeInformation.getNeJobId());
                systemRecorder.recordCommand(SHMEvents.STN_UPGRADE_SERVICE, CommandPhase.ONGOING, stnUpgradeInformation.getNeName(), stnUpgradeInformation.getNodeFdn(),
                        StnJobConstants.UPGRADE_ACTIVITY);

            } else {
                activityUtils.prepareJobLogAtrributesList(jobLogs, "Activate Activity is Failed ",
                        (Date) notification.getCommonNotification().getAdditionalAttributes().get(StnJobConstants.NOTIFICATION_TIME_STAMP), JobLogType.SYSTEM.toString(), JobLogLevel.INFO.toString());
                ...
                systemRecorder.recordCommand(SHMEvents.STN_UPGRADE_SERVICE, CommandPhase.FINISHED_WITH_ERROR, stnUpgradeInformation.getNeName(), stnUpgradeInformation.getNodeFdn(),
                        StnJobConstants.UPGRADE_ACTIVITY);
                LOGGER.debug("ActivityJob ID - [{}] : Create Software Upgrade Job has been completed successfully. Notifying to workflow service.", activityJobId);
    ...
    public ActivityStepResult handleTimeout(final long activityJobId) {
        ...
        if (stnUpgradeInformation == null) {
            LOGGER.error("Unable to fetch the required information of the activityId {} ", activityJobId);
            final String logMessage = String.format(JobLogConstants.ACTIVITY_FAILED, ActivityConstants.UPGRADE);
            activityUtils.prepareJobLogAtrributesList(jobLogs, logMessage, new Date(), JobLogType.SYSTEM.toString(), JobLogLevel.ERROR.toString());
            stnJobActivityService.failActivity(activityJobId, jobLogs, activityUtils.getJobEnvironment(activityJobId), ActivityConstants.UPGRADE, new HashMap<String, Object>());
        ...
        final String logMessage = String.format(StnJobConstants.ACTIVITY_IN_TIMEOUT, StnJobConstants.UPGRADE_ACTIVITY);
        activityUtils.prepareJobLogAtrributesList(jobLogs, logMessage, new Date(), JobLogType.SYSTEM.toString(), JobLogLevel.ERROR.toString());
        jobUpdateService.readAndUpdateRunningJobAttributes(activityJobId, jobProperties, jobLogs);
        stnSoftwareEndUpgradeEventSender.sendSoftwareUpgradeActionRequest(activityJobId, StnJobConstants.UPGRADE_ACTIVITY, nodeAddress, eventAttributes);
        activityUtils.unSubscribeToMoNotifications(subscriptionKey, activityJobId, jobActivityInformation);
        return activityStepResult;
    }
    ...
    public ActivityStepResult cancel(final long activityJobId) {
        final String actionTriggered = StnJobConstants.UPGRADE_ACTIVITY;
