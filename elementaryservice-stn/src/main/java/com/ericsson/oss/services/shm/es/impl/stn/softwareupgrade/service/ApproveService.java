...
import com.ericsson.oss.services.shm.es.impl.stn.softwareupgrade.common.StnSoftwareUpgradeApproveEventSender;
...
/**
 * This class performs Confirm/ApproveSw activity of Software Upgrade on STN nodes.
 * 
 * @author xsamven/xvenupe
 * 
 */
@EServiceQualifier("STN.UPGRADE.approvesw")
@ActivityInfo(activityName = "approvesw", jobType = JobTypeEnum.UPGRADE, platform = PlatformTypeEnum.STN)
@Stateless
@Traceable
@Profiled
@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
public class ApproveService implements Activity, ActivityCallback {
    ...
    @Inject
    private StnSoftwareUpgradeApproveEventSender stnSoftwareUpgradeEventSender;
    ...
    /**
     * Precheck for confirm activity of Software Upgrade on STN nodes.
     * 
     * @param activityJobId
    ...
    public ActivityStepResult precheck(final long activityJobId) {
        ...
        try {
            ...
            //TBAC Validation
            final boolean isUserAuthorized = activityJobTBACValidator.validateTBAC(activityJobId, neJobStaticData, jobStaticData, ActivityConstants.APPROVE_SW);
            if (!isUserAuthorized) {
                activityStepResult.setActivityResultEnum(ActivityStepResultEnum.PRECHECK_FAILED_SKIP_EXECUTION);
                return activityStepResult;
            }
            activityUtils.prepareJobLogAtrributesList(jobLogs, String.format(JobLogConstants.PRECHECK_SUCCESS, ActivityConstants.APPROVE_SW), new Date(), JobLogType.SYSTEM.toString(),
            ...
            LOGGER.info("ActivityJob ID - [{}] : Precheck of ApproveSw activity is completed. Result : {}", activityJobId, activityStepResult.getActivityResultEnum());
        } catch (Exception exception) {
            LOGGER.error("Exception occured in precheck() for activityJobId:{}. Reason: ", activityJobId, exception);
            activityStepResult.setActivityResultEnum(ActivityStepResultEnum.PRECHECK_FAILED_SKIP_EXECUTION);
            jobLogUtil.prepareJobLogAtrributesList(jobLogs, String.format(JobLogConstants.PRE_CHECK_FAILURE, ActivityConstants.APPROVE_SW, exception.getMessage()), new Date(),
    ...
    /**
     * Method to register the notifications, initiate and perform the action on STN Node
     * 
     * @param activityJobId
     * 
     */
    ...
    public void execute(final long activityJobId) {
        ...
        try {
            ...
            if (stnUpgradeInformation == null) {
                ...
                logMessage = String.format(JobLogConstants.ACTIVITY_FAILED, ActivityConstants.APPROVE_SW);
                ...
                stnJobActivityService.failActivity(activityJobId, jobLogs, activityUtils.getJobEnvironment(activityJobId), ActivityConstants.APPROVE_SW, processVariables);
                return;
            }
            LOGGER.info("ActivityJob ID - [{}] : Entered execute of ApproveSw activity of software upgrade on node:{}", activityJobId, stnUpgradeInformation.getNodeFdn()); //Add Node Name
            jobContext = stnUpgradeInformation.getJobEnvironment();

            systemRecorder.recordCommand(SHMEvents.STN_APPROVE_SERVICE, CommandPhase.STARTED, stnUpgradeInformation.getNeName(), stnUpgradeInformation.getNodeFdn(),
                    StnJobConstants.APPROVE_SW_ACTIVITY);
            ...
            activityUtils.prepareJobLogAtrributesList(jobLogs, String.format(JobLogConstants.ACTIVITY_INITIATED, ActivityConstants.APPROVE_SW), new Date(), JobLogType.SYSTEM.toString(),
                    JobLogLevel.INFO.toString());
            LOGGER.debug("ActivityJob ID - [{}] : Execute of ApproveSw activity is triggered successfully.", activityJobId);
            ...
            logMessage = String.format(JobLogConstants.ACTIVITY_FAILED, ActivityConstants.APPROVE_SW);
            activityUtils.prepareJobLogAtrributesList(jobLogs, logMessage, new Date(), JobLogType.SYSTEM.toString(), JobLogLevel.ERROR.toString());
            stnJobActivityService.failActivity(activityJobId, jobLogs, jobContext, ActivityConstants.APPROVE_SW, processVariables);
        } catch (final Exception exception) {
            LOGGER.error("Failed to execute ApproveSw activity and cannot be proceeded with activity with activityId {}. for Node :{} Reason : {}", activityJobId, stnUpgradeInformation.getNodeFdn(),
                    exception);
            logMessage = String.format(JobLogConstants.ACTIVITY_FAILED, ActivityConstants.APPROVE_SW);
            activityUtils.prepareJobLogAtrributesList(jobLogs, logMessage, new Date(), JobLogType.SYSTEM.toString(), JobLogLevel.ERROR.toString());
            stnJobActivityService.failActivity(activityJobId, jobLogs, jobContext, ActivityConstants.APPROVE_SW, processVariables);
    ...
    private void sendSoftwareUpgradeActivityRequest(final long activityJobId, final StnJobInformation stnUpgradeInformation, final JobActivityInfo jobActivityInformation) {
        final List<Map<String, Object>> jobLogs = new ArrayList<>();
        LOGGER.debug("ActivityJob ID - [{}] : ApproveSw activity of software upgrade job is triggered. Software UpgradeJob Information : {} ", activityJobId, stnUpgradeInformation);
        activityUtils.prepareJobLogAtrributesList(jobLogs, String.format(StnJobConstants.ACTION_ABOUT_TO_TRIGGER, ActivityConstants.APPROVE_SW, stnUpgradeInformation.getNodeFdn()), new Date(),
        ...
        final String subscriptionKey = StnJobConstants.SUBSCRIPTION_KEY_DELIMETER + stnUpgradeInformation.getNodeFdn() + StnJobConstants.SUBSCRIPTION_KEY_DELIMETER
                + StnJobConstants.APPROVE_SW_ACTIVITY + StnJobConstants.SUBSCRIPTION_KEY_DELIMETER;
        ...
        if (nodeFdn != null) {
            ...
            LOGGER.debug("ActivityJob ID - [{}] : Subscription key for ApproveSw action is : {}", activityJobId, subscriptionKey);
            activityUtils.subscribeToMoNotifications(subscriptionKey, activityJobId, jobActivityInformation);
            jobUpdateService.readAndUpdateRunningJobAttributes(activityJobId, null, jobLogs);
            stnSoftwareUpgradeEventSender.sendSoftwareUpgradeActionRequest(activityJobId, ActivityConstants.APPROVE_SW, nodeFdn, eventAttributes);
        } else {
            jobUpdateService.readAndUpdateRunningJobAttributes(activityJobId, null, jobLogs);
    ...
    /**
     * This method processes the notifications by fetching the notification subject and validates the notification.
     * 
     * @param message
     * 
     */
    ...
    private void handleActivityProgressNotification(final SHMCommonCallBackNotificationJobProgressBean notification, final StnJobInformation stnUpgradeInformation,
        ...
        LOGGER.debug("ActivityJob ID - [{}] : Job notification received for ApproveSw activity is: {}", activityJobId, notification.getCommonNotification());
        eventAttributes.put(ShmConstants.NE_NAME, stnUpgradeInformation.getNeName());
        eventAttributes.put(ActivityConstants.ACTIVITY_JOB_ID, activityJobId);
        ...
        try {
            ...
            if (ShmConstants.SUCCESS.equalsIgnoreCase(result)) {
                activityUtils.prepareJobLogAtrributesList(jobLogs, "Approve Activity is Successful ",
                ...
                activityUtils.sendNotificationToWFS(jobContext, jobActivityInformation.getActivityJobId(), ActivityConstants.ACTIVATE, processVariables);
                systemRecorder.recordCommand(SHMEvents.STN_APPROVE_SERVICE, CommandPhase.FINISHED_WITH_SUCCESS, stnUpgradeInformation.getNeName(), stnUpgradeInformation.getNodeFdn(),
                        StnJobConstants.APPROVE_SW_ACTIVITY);
                ...
                LOGGER.info("getActivityResult : {} , progressPercent : {}", result ,progressPercent);
                activityAndNEJobProgressPercentageCalculator.updateActivityJobProgressPercentage(activityJobId, jobProperties, jobLogs, progressPercent);
                activityAndNEJobProgressPercentageCalculator.updateNEJobProgressPercentage(stnUpgradeInformation.getNeJobId());
                systemRecorder.recordCommand(SHMEvents.STN_APPROVE_SERVICE, CommandPhase.ONGOING, stnUpgradeInformation.getNeName(), stnUpgradeInformation.getNodeFdn(),
                        StnJobConstants.APPROVE_SW_ACTIVITY);

            } else {
                activityUtils.prepareJobLogAtrributesList(jobLogs, "Approve Activity is Failed ",
                        (Date) notification.getCommonNotification().getAdditionalAttributes().get(StnJobConstants.NOTIFICATION_TIME_STAMP), JobLogType.SYSTEM.toString(), JobLogLevel.INFO.toString());
                ...
                systemRecorder.recordCommand(SHMEvents.STN_APPROVE_SERVICE, CommandPhase.FINISHED_WITH_ERROR, stnUpgradeInformation.getNeName(), stnUpgradeInformation.getNodeFdn(),
                        StnJobConstants.APPROVE_SW_ACTIVITY);
                LOGGER.debug("ActivityJob ID - [{}] : Create Software Upgrade Job has been completed successfully. Notifying to workflow service.", activityJobId);
            }
            ...
            final String jobLogMessage = String.format(JobLogConstants.ACTIVITY_FAILED, ActivityConstants.APPROVE_SW);
    ...
    /**
     * Method to handle the job execution in timeout scenario.
     */
    @SuppressWarnings("deprecation")
    @Override
    public ActivityStepResult handleTimeout(final long activityJobId) {
        ...
        LOGGER.debug("ActivityJob ID - [{}] : Handling ApproveSw Activity of software upgrade in timeout.", activityJobId);
        final StnJobInformation stnUpgradeInformation = stnSoftwareUpgradeActivityService.buildStnUpgradeInformation(activityJobId);
        if (stnUpgradeInformation == null) {
            LOGGER.error("Unable to fetch the required information of the activityId {} ", activityJobId);
            final String logMessage = String.format(JobLogConstants.ACTIVITY_FAILED, ActivityConstants.APPROVE_SW);
            activityUtils.prepareJobLogAtrributesList(jobLogs, logMessage, new Date(), JobLogType.SYSTEM.toString(), JobLogLevel.ERROR.toString());
            stnJobActivityService.failActivity(activityJobId, jobLogs, activityUtils.getJobEnvironment(activityJobId), ActivityConstants.APPROVE_SW, new HashMap<String, Object>());
        ...
        final String logMessage = String.format(StnJobConstants.ACTIVITY_IN_TIMEOUT, ShmConstants.CONFIRM_ACTIVITY);
        activityUtils.prepareJobLogAtrributesList(jobLogs, logMessage, new Date(), JobLogType.SYSTEM.toString(), JobLogLevel.ERROR.toString());
        jobUpdateService.readAndUpdateRunningJobAttributes(activityJobId, jobProperties, jobLogs);
        stnSoftwareEndUpgradeEventSender.sendSoftwareUpgradeActionRequest(activityJobId, StnJobConstants.APPROVE_SW_ACTIVITY, nodeAddress, eventAttributes);
        activityUtils.unSubscribeToMoNotifications(subscriptionKey, activityJobId, jobActivityInformation);
        return activityStepResult;
    }

    /**
     * Method to cancel the software upgrade job execution in Confirm activity.
     * 
     */
    @Override
    public ActivityStepResult cancel(final long activityJobId) {
        ...
        return cancelSoftwareUpgradeService.cancel(activityJobId, StnJobConstants.APPROVE_SW_ACTIVITY, activityUtils.getActivityInfo(activityJobId, this.getClass()));
    }
