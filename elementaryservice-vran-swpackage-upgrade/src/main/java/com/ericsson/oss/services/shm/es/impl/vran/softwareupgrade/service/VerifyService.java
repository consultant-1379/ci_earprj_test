...
/**
 * This class performs Verify activity of Software Upgrade job on vRAN nodes.
 *
 * @author xragmou
 *
 */
@EServiceQualifier("vRAN.UPGRADE.verify")
...
public class VerifyService implements Activity, ActivityCallback {
    ...
    public ActivityStepResult precheck(final long activityJobId) {
        LOGGER.info("ActivityJob ID - [{}] : precheck of Verify activity of software upgrade on node.", activityJobId);
        return precheckTask.activityPreCheck(activityJobId, ActivityConstants.VERIFY);
    }

    @Override
    @Asynchronous
    public void execute(final long activityJobId) {
        LOGGER.info("ActivityJob ID - [{}] : Executing Verify activity of software upgrade on node.", activityJobId);
    ...
    public void processNotification(final Notification message) {
        ...
        LOGGER.info("ActivityJob ID - [{}] : Notification received from mediation for Verify activity is {}", activityJobId, vranSoftwareUpgradeJobResponse);
        ...
        verifyActivityNotificationProcessor.processNotification(jobActivityInformation, vranSoftwareUpgradeJobResponse, upgradePackageContext);
    }

    @Override
    public ActivityStepResult handleTimeout(final long activityJobId) {
        LOGGER.info("ActivityJob ID - [{}] : Handling Verify Activity of software upgrade in timeout.", activityJobId);
        final JobActivityInfo jobActivityInformation = activityUtils.getActivityInfo(activityJobId, this.getClass());
    ...
    public ActivityStepResult cancel(final long activityJobId) {
        ...
        final JobActivityInfo jobActivityInformation = activityUtils.getActivityInfo(activityJobId, this.getClass());
        return verifyActivityCancelProcessor.processCancelForUnSupportedActivity(jobActivityInformation);
    }
