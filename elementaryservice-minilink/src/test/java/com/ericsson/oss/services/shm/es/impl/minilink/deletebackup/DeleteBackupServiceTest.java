...
public class DeleteBackupServiceTest {
    ...
    private static final String DELETEBACKUP_CANCEL_LOG = "Cancel request received for \"deletebackup\" by \"\".";
    ...
    private final InjectionProperties injectionProperties = new InjectionProperties().autoLocateFrom("com.ericsson.oss.services.shm.es.impl.minilink.deletebackup");
    @Rule
    ...
    public void setup() {
        ...
        when(ne.getPlatformType()).thenReturn(PlatformTypeEnum.MINI_LINK_INDOOR);
        when(ne.getNeType()).thenReturn(PlatformTypeEnum.MINI_LINK_INDOOR.getName());
    ...
    public void testPrecheck() {
        ...
        verify(jobUpdateService).readAndUpdateRunningJobAttributes(eq(ACTIVITY_JOB_ID), Matchers.anyList(), jobLogListCaptor.capture());
    ...
    public void testExecuteDeleteSuccessfully() {
        ...
        when(deleteSmrsBackupService.deleteBackupOnSmrs(NODE_NAME, BACKUP_NAME_TEST, PlatformTypeEnum.MINI_LINK_INDOOR.getName())).thenReturn(true);

        deleteBackupService.execute(ACTIVITY_JOB_ID);

        verify(deleteSmrsBackupService).deleteBackupOnSmrs(NODE_NAME, BACKUP_NAME_TEST, PlatformTypeEnum.MINI_LINK_INDOOR.getName());
        verify(jobUpdateService, times(3)).readAndUpdateRunningJobAttributes(eq(ACTIVITY_JOB_ID), jobPropertyListCaptor.capture(), jobLogListCaptor.capture());
    ...
    public void testExecuteCannotDelete() {
        ...
        when(deleteSmrsBackupService.deleteBackupOnSmrs(NODE_NAME, BACKUP_NAME_TEST, PlatformTypeEnum.MINI_LINK_INDOOR.getName())).thenReturn(false);
    ...
    public void testCancel() {
        ...
        assertEquals(1, jobLogList.size());
        assertEquals(DELETEBACKUP_CANCEL_LOG, jobLogList.get(0).get("message"));
    ...
    private final JobStaticDataProvider jobStaticDataProvider = new JobStaticDataProvider() {
        ...
        public JobStaticData getJobStaticData(final long mainJobId) {
            final JobStaticData jobStaticData = new JobStaticData("", new HashMap<String, Object>(), "", JobType.DELETEBACKUP,"");
